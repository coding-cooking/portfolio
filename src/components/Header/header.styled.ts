...
export const Header = styled.header<HeaderInterface>`
	...
	box-shadow: ${(props) =>
		props.isShowShadow ? "0 6px 16px -12px var(--box-shadow)" : "0 1px 6px -6px var(--box-shadow)"};
`;
...
export const LanguageSpan = styled.span<LanguageActive>`
    ...
    cursor: pointer;
	&:first-child {
		border-right: ${props => props.active ? '1px solid var(--nav-color)' : '0 none'};
	}
    &:last-child {
        border-left: ${props => props.active ? '1px solid var(--nav-color)' : '0 none'}; 
    }
`;
